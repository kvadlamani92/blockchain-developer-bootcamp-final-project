...
export const useNFTTokenIds = (addr) => {
  ...
  console.log('chainId',chainId,'addr',addr); 
  const {
    ...
    if (data?.result) {
      ...
      for (let NFT of NFTs) {
        if (NFT?.metadata) {
          ...
          NFT.contract_type = NFT.metadata.description;
 ...
 */
  ...
  console.log('getNFTTOkenIds:',getNFTTokenIds,'NFTTokenIDs:',NFTTokenIds,'totalNFTs',totalNFTs,'fetchSuccess:', fetchSuccess);
  return {
    getNFTTokenIds,
    NFTTokenIds,
    totalNFTs,
